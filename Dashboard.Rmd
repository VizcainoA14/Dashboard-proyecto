...
---------------
### Conclusiones
Luego de realizar la graficas nos podemos dar cuenta que la variable appeared se comporta como categorica, por lo que no tendria sentido analizar los estadisticos para verificar su dependencia, sin embargo con fines de estudio, verificacion y practica, a partir de los estadisticos podemos conlcuir lo siguiente

* Tanto la suma de los errores como la variabilidad del modelo es muy alta, lo cual indica independencia entre las variables

* La correlación es −0.0113942352910447, muy cercana a cero, lo que sugiere una relación muy débil o casi inexistente entre las variables
...
---------------
### Conclusiones
Luego de realizar la graficas nos podemos dar cuenta que la variable lastactivity se comporta como categorica, por lo que no tendria sentido analizar los estadisticos para verificar su dependencia, sin embargo con fines de estudio, verificacion y practica, a partir de los estadisticos podemos conlcuir lo siguiente

* Tanto la suma de los errores como la variabilidad del modelo es muy alta, lo cual indica independencia entre las variables

* La correlación es 	0.0559661030339999, muy cercana a cero, lo que sugiere una relación muy débil o casi inexistente entre las variables
...
---------------
### Conclusiones
Basándonos en las gráficas y lo que hemos estudiado en nuestro curso de Estadística Inferencial, podriamos afrimar que:

* los tipos de tecnologia que tienen mas rango de apariciones son las de: "pl", "querylanguaje" y "xmlformat"
...
* Hay un amplio rango de años de actividad para los diferentes tipos de software, con algunos tipos que tienen un nivel de actividad mediano más alto que otros.

* Los tipos de software con los niveles de actividad medianos más altos son “pl” y “protocol”.
...
---------------
### Conclusiones
la variable numbeofUsers presenta una gran variabilidad por lo cual los boxplot a escala normal se vel muy pequeños, sin embargo, usando la escala log10 podriamos afirmar que:

* la tecnologia que tiene mas usuarios es alguna de tipo "pl"
...
---------------
### Conclusiones
la variable numbeofUsers presenta una gran variabilidad por lo cual los boxplot a escala normal se ven muy pequeños, sin embargo, usando la escala log10 podriamos afirmar que:

* el pais que tiene la tecnologia con mas usuarios es United states
...
---------------
### Conclusiones
A partir de grafico boxplot con categorias podriamos afirmar que

* el pais de la tecnologia que menos se actualiza es "Unknow"
...
---------------
### Conclusiones
Basándonos en las gráficas y lo que hemos estudiado en nuestro curso de Estadística Inferencial, podemos afirmar que:

* los paises que han producido tecnologias de 1960 hasta 2022 son: "United States" y "United Kingdom"
...
* El valor mediano para el grupo “YES” es igual que el valor mediano para el grupo “NO”.
...
---------------
### Conclusiones
Realizando una analisis de las graficas podemos afirmar que:

* Tanto YES como NO tiene el mismo rango de valores
...
* la tecnologia que mas tiene ususarios no tiene sitio web propio
...
TYPE - COUNTRY{data-navmenu="Analisis bivariado"}
=======================
Rows {data-width=600, .tabset}
-----------------------------------------------------------------------
...
### Gráfico de Barras: Tipo por País
```{r}

# Supongamos que ya tienes la tabla generada
tabla_type_country <- table(Muestra$type, Muestra$country)

# Convertimos la tabla a un dataframe
df <- as.data.frame.matrix(tabla_type_country)

# Añadimos una columna con los tipos
df$type <- rownames(df)

# Transformamos los datos a formato largo para ggplot2
df_long <- tidyr::gather(df, key = "Country", value = "Count", -type)

# Creamos el gráfico de barras
x = ggplot(df_long, aes(x = type, y = Count, fill = Country)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Gráfico de Barras: Tipo por País", x = "Type", y = "Cantidad") +
  theme_minimal()+scale_y_log10()+theme(axis.text.x = element_blank())
...
---------------
### Conclusiones
Para probar la dependencia entre las dos variables categoricas debemos realizar la prueba ji cuadrada la cual nos dio el siguiente resultado:
```{r}
tabla_contingencia <- table(Muestra$type, Muestra$country)
...
```
Con este resultado rechazamos la hipotesis nula de que no hay diferencias significativas en la distribución de género entre los dos grupos.
...
### Estadisticos
```{r}
DT::datatable(data = Muestra[, c(1, 3, 7)], 
              extensions = 'Scroller',
              options = list(dom='tp',  
                             class = 'stripe compact hover cell-border'))
...
WEBSITE - TYPE{data-navmenu="Analisis bivariado"}
=======================
Rows {data-width=600, .tabset}
-----------------------------------------------------------------------
### tabla de contingencia
```{r}
tabla_type_country <- table(Muestra$type, Muestra$website)
kable(tabla_type_country)
...
# Supongamos que ya tienes la tabla generada
tabla_type_country <- table(Muestra$website, Muestra$type)
...
x = ggplot(df_long, aes(x = type, y = Count, fill = Country)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Gráfico de Barras: Tipo por tecnologia", x = "Website", y = "Cantidad") +
...
Para probar la dependencia entre las dos variables categoricas debemos realizar la prueba ji cuadrada la cual nos dio el siguiente resultado:
```{r}
tabla_contingencia <- table(Muestra$website, Muestra$type)
...
### Estadisticos
```{r}
DT::datatable(data = Muestra[, c(1, 3, 6)], 
              extensions = 'Scroller',
              options = list(dom='tp',  
                             class = 'stripe compact hover cell-border'))

```
COUNTRY - WEBSITE{data-navmenu="Analisis bivariado"}
=======================
Rows {data-width=600, .tabset}
-----------------------------------------------------------------------
### tabla de contingencia
```{r}
tabla_type_country <- table(Muestra$country, Muestra$website)
kable(tabla_type_country)
...
# Supongamos que ya tienes la tabla generada
tabla_type_country <- table(Muestra$website, Muestra$country)
...
x = ggplot(df_long, aes(x = type, y = Count, fill = Country)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Gráfico de Barras: Tipo por País", x = "Website", y = "Cantidad") +
  theme_minimal()+scale_y_log10()
...
Para probar la dependencia entre las dos variables categoricas debemos realizar la prueba ji cuadrada la cual nos dio el siguiente resultado:
```{r}
tabla_contingencia <- table(Muestra$website, Muestra$country)
...
### Estadisticos
```{r}
DT::datatable(data = Muestra[, c(1, 7, 6)], 
              extensions = 'Scroller',
              options = list(dom='tp',  
                             class = 'stripe compact hover cell-border'))
